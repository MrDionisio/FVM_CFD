Subroutine B_CalcDiv(NI,NJ,CellCenter,CellVolume,IFaceCenter,IFaceVector,JFaceCenter,JFaceVector,&
    V,DivV,P,GradP,scheme)
    implicit none
    REAL CellCenter(0:NI,0:NJ,2),CellVolume(NI-1,NJ-1),& ! центры и объемны ячеек
    ...
    real Sf(4,2),rf(4,2),Vf(2),Pf(4)
    integer NeighCell(4,2)
    real d,dn, RLinearInterp
    integer i,j,inc,jnc,iface,scheme, NI, NJ
                ...
                select case(scheme)
                    ...
                    case(3)
                        ...
                        if (dot_product(Vf(:),Sf(iface,:)) > 0.0) then
                            ...
                            Pf(iface) = P(inc,jnc) + dot_product(GradP(inc,jnc,:),rf(iface,:) - CellCenter(inc,jnc,:))
                            ! учет приграничных ячеек
                            if (dn < 1e-7) then
                                Pf(iface) = 2*P(inc,jnc) - P(i,j) - 4*P(inc,jnc) + 4*P(i,j)&
