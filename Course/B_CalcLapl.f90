Subroutine B_CalcLapl(NI,NJ,CellCenter,CellVolume,IFaceCenter,IFaceVector,JFaceCenter,JFaceVector,&
    P,GradP,LaplP)
    ...
    real Sf(4,2),rf(4,2),GPE(2),rE(2),Pf(4),rnc(2),Nf(2)
    ...
    do i = 1,NI-1
        do j = 1,NJ-1
            ...
            do iface = 1,4
                ...
                dn = norm2(rf(iface,:) - CellCenter(inc,jnc,:)) ! Расст до центра соседней ячейки
                Pf = RLinearInterp(d,dn,P(i,j),P(inc,jnc))

                ! Расстояние между 2 соседн ячейками (модуль вект)
                dnc = norm2(CellCenter(inc,jnc,:) - CellCenter(i,j,:))
                dpdn = (P(inc,jnc) - P(i,j))/dnc ! произв по напр к соседн
                ...
                ! Случай для приграничной ячейки
                if (dn.lt.1e-7) then
