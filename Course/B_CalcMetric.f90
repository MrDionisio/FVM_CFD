SUBROUTINE B_CalcMetric(NI,NJ,X,Y,CellCenter,CellVolume,IFaceCenter,IFaceVector,JFaceCenter,JFaceVector)
    REAL X(NI,NJ),Y(NI,NJ),& ! входные: координаты узлов сетки
     CellCenter(0:NI,0:NJ,2),CellVolume(NI-1,NJ-1),& ! выходные: центры ячеек и объемыячеек
     IFaceCenter( NI,NJ-1,2),IFaceVector( NI,NJ-1,2),& ! центры и векторы граней для Iграней
     JFaceCenter( NI-1,NJ,2),JFaceVector( NI-1,NJ,2) ! центры и векторы граней для Jграней
    ...
    CellVolume(I,J) = 0.5*DOT_PRODUCT(IFaceVector(I,J,:),r)& ! скалярноплощадь грани x диаг
