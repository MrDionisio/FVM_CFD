Subroutine B_CalcResT(NI,NJ,CellCenter,CellVolume,IFaceCenter,IFaceVector,JFaceCenter,JFaceVector,&
    ...
    REAL CellCenter(0:NI,0:NJ,2),CellVolume(NI-1,NJ-1),& ! центры и объемы ячеек
     ...
     JFaceCenter( NI-1,NJ,2),JFaceVector( NI-1,NJ,2),& ! центры и нормали граней дляJ-граней
    ...
    do j = 1,NJ-1
        do i = 1,NI-1
            ...
            do iface = 1,4
                ...
                dn = norm2(rf(iface,:) - CellCenter(inc,jnc,:)) ! расст до центра соседней яч

                ! расчет конвективного слагаемого
                Vf(1) = RLinearInterp(d,dn,VFlos(i,j,1),VFlos(inc,jnc,1)) ! лин интерп скоростина грани
                ...
                ! Итоговое конвективное слагаемое
                VTf(:) = Vf(:)*Tf
                ...
                rnc = (CellCenter(inc,jnc,:) - CellCenter(i,j,:))/dnc ! Поправка на неортогональность
